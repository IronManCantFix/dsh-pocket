...
import { startFrpTunnel } from './frp-tunnel.mjs';
...
export function detectWsl() {
  ...
  // WSLENV（如 WT_SESSION:WT_PROFILE_ID:），会误判成 WSL 而错误地走 ipconfig 分支。
  // 只认 WSL 内部才有的变量（PR #62）。
...
export function createPocketService({
  ...
  onTunnelReady,
  /** NAS 反向隧道（frp）配置读取：返回 {serverAddr, serverPort, remotePort, tls, token}；null 表示未启用 */
  getFrpConfig,
  /** 局域网访问总开关：返回 boolean；默认开启（PR #61）。关闭后代理拒绝局域网 Host 请求。
   *  每次请求实时读，设置页切一下立即生效，不用重启代理。 */
  ...
  let frp = null;
  let frpAbort = null;
  /** in-flight frp 隧道启动（单飞）：并发调用复用同一次，避免 spawn 多个 frpc 孤儿进程 */
  let frpPromise = null;
  /** frp 隧道进度：{ phase: idle|downloading|starting|connecting|ready|error, detail, startedAt } */
  const frpState = { phase: 'idle', detail: '', startedAt: null };
  ...
  // 自动恢复标记的存放根目录：必须与配置持久化（settings.json / frp-token，见 settings.mjs）
  // 保持同一 base，否则「开启时写标记」与「重启后读标记」会因 internals.home 未传入而断裂
  // （真实部署下 DSH 未必提供 internals.home）。优先级：显式 home → $DSH_HOME → ~/.dsh。
  const storageBase = home ?? process.env.DSH_HOME ?? join(homedir(), '.dsh');

  // 公网隧道自动恢复（issue #11）：DSH 重启后 cloudflared 子进程被杀、隧道消失，
  // 插件无从知晓。启动时检查持久化的「隧道开启中」标记，自动重新拉起。
  const autoStatePath = join(storageBase, 'dsh-pocket', 'tunnel-auto.json');
  ...
  // frp 隧道自动恢复标记（与 cloudflared 的 tunnel-auto.json 平行，互不干扰）
  const frpAutoStatePath = join(storageBase, 'dsh-pocket', 'tunnel-auto-frp.json');
  // 同理串行化，避免「刚就绪就关闭」时晚到的写把标记复活
  let frpAutoStateQueue = Promise.resolve();
  function persistAutoFrp() {
    frpAutoStateQueue = frpAutoStateQueue.then(async () => {
      try {
        await mkdir(dirname(frpAutoStatePath), { recursive: true });
        await writeFile(frpAutoStatePath, JSON.stringify({ at: Date.now() }), 'utf8');
      } catch { /* 忽略 */ }
    });
    return frpAutoStateQueue;
  }
  function clearAutoFrp() {
    frpAutoStateQueue = frpAutoStateQueue.then(async () => {
      try { await rm(frpAutoStatePath, { force: true }); } catch { /* 忽略 */ }
    });
    return frpAutoStateQueue;
  }

  /** 读取 frp 配置（含 token）；未配置完整时返回 null。 */
  function frpConfigOrNull() {
    const c = getFrpConfig?.() ?? null;
    if (!c) return null;
    if (!c.serverAddr || !c.token) return null;
    return c;
  }
  ...
  return {
      ...
      for (let p = port; p < port + 10; p++) {
        try {
          proxy = await createProxy({
            ...
            // 局域网访问总开关（PR #61）：每次请求实时读，切开关立即生效
    ...
    /**
     * 停止公网隧道（代理保持）。
     * @param {{ keepAutoMarker?: boolean }} [opts] - keepAutoMarker=true 用于进程退出/自重启
     *   （dispose / pocketRestart）：隧道随进程被动消失，不是用户主动关闭，必须保留
     *   tunnel-auto.json，下次启动才能自动恢复（issue #11 的本意）；默认（设置页手动关闭）
     *   删除标记。
     */
    ...
    /** 启动 NAS 反向隧道（frp，幂等；配置不完整/无 token 时抛错）。进度写进 frpState。并发调用单飞。 */
    async startFrpTunnel() {
      await this.startProxy();
      const cfg = frpConfigOrNull();
      if (!cfg) throw new Error('请先填写 NAS 地址并设置连接令牌 | configure the NAS address and token first');
      if (frp) return frp.ready;
      if (frpPromise) return frpPromise; // 复用 in-flight，防孤儿 frpc
      const controller = new AbortController();
      frpAbort = controller;
      frpState.startedAt = Date.now();
      const onPhase = (phase, detail) => {
        frpState.phase = phase;
        if (detail) frpState.detail = detail;
        else if (phase === 'downloading') frpState.detail = '首次下载 frpc（约 10MB）| first run downloads frpc (~10MB)';
        else if (phase === 'starting') frpState.detail = '启动 frpc 进程… | starting frpc…';
        else if (phase === 'connecting') frpState.detail = '连接 NAS frps（通常数秒）| connecting to NAS frps';
        else if (phase === 'ready') frpState.detail = '隧道就绪 | ready';
      };
      // 与 startTunnel 同款 TDZ 防护：先 `await null` 让函数体从微任务开始执行，
      // 否则在 `p` 初始化之前（startFrp 同步抛错）就会走到 finally 引用未初始化的 const，
      // frpPromise 永久停在 rejected → NAS 反向隧道再也起不来。
      const p = (async () => {
        await null;
        try {
          const handle = await startFrp({
          ...
          // frpc 进程运行中死亡（崩溃/被杀）→ 状态打回，别让 UI 永远显示"可用"
          handle.onExit?.((code) => {
            if (controller.signal.aborted) return; // 主动停止（stopFrpTunnel）不算故障
            frpState.phase = 'error';
            frpState.detail = `frpc 进程退出（code=${code}）| frpc process exited`;
          });
          frp = handle;
          frpState.phase = 'ready';
          // 记录「隧道开启中」，供重启后自动恢复
          void persistAutoFrp();
          return true;
        } catch (err) {
          if (!controller.signal.aborted) {
            frpState.phase = 'error';
            frpState.detail = err?.message ?? String(err);
          }
          frpState.startedAt = null;
          throw err;
        } finally {
          // 只清自己的引用（与 startTunnel 同款竞态处理）
          if (frpPromise === p) frpPromise = null;
        }
      })();
      frpPromise = p;
      return p;
    ...
    /**
     * 停止 NAS 反向隧道（代理保持）。
     * @param {{ keepAutoMarker?: boolean }} [opts] - 语义同 stopTunnel：进程退出/自重启
     *   （dispose / pocketRestart）保留标记，用户手动关闭删除标记。
     */
    stopFrpTunnel({ keepAutoMarker = false } = {}) {
      frpAbort?.abort();
      frpAbort = null;
      frpPromise = null;
      if (frp) { try { frp.kill?.(); } catch { /* 忽略 */ } }
      frp = null;
      frpState.phase = 'idle';
      frpState.detail = '';
      frpState.startedAt = null;
      if (!keepAutoMarker) void clearAutoFrp(); // 手动关闭后不再自动恢复
    },

    /** 启动时自动恢复上次开启的 frp 隧道（DSH 重启后 frpc 子进程被杀）。 */
    async restoreFrpIfNeeded() {
      logInfo('dsh-pocket: frp auto-restore check | frp 自动恢复检查：读取上次隧道状态');
      if (frp || frpPromise) return;
      let has = false;
      try {
        const raw = await readFile(frpAutoStatePath, 'utf8');
        has = /"at"\s*:/.test(raw);
      } catch { return; } // 无标记 → 不恢复
      if (!has) return;
      if (!frpConfigOrNull()) {
        logWarn('dsh-pocket: frp auto-restore skipped — config incomplete | frp 自动恢复跳过：配置不完整');
        return;
      }
      logInfo('dsh-pocket: frp auto-restore: marker found, starting tunnel | 发现上次开启标记，尝试自动恢复');
      try {
        await this.startFrpTunnel();
        logInfo('dsh-pocket: frp tunnel auto-restored | 已自动恢复 NAS 反向隧道');
      } catch (err) {
        logWarn('dsh-pocket: frp auto-restore failed | 自动恢复 frp 隧道失败: %s', err?.message ?? err);
      }
    },
    ...
    async status() {
      ...
      return {
        ...
        // NAS 反向隧道（frp）：只暴露非敏感字段，token 永不外传
        frpRunning: frp !== null,
        frpState: { ...frpState },
        frpConfig: frpCfg ? { serverAddr: frpCfg.serverAddr, serverPort: frpCfg.serverPort, remotePort: frpCfg.remotePort, tls: frpCfg.tls === true } : null,
        frpHasToken: Boolean(frpCfg?.token),
        dshPort,
      };
    },
    ...
    /**
     * 停止一切（插件卸载时）。
     * 进程退出不是用户手动关闭：保留自动恢复标记，否则 DSH 每次正常重启都会把
     * 「重启后自动恢复公网/NAS 隧道」清掉（issue #106）。
     */
    async dispose() {
      this.stopTunnel({ keepAutoMarker: true });
      this.stopFrpTunnel({ keepAutoMarker: true });
      if (proxy) {
        const p = proxy;
        proxy = null;
