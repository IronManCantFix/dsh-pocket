...
/**
 * 该 host 上被接受的全部密码：主 PIN + 可选替代令牌（`auth.getAltTokens(host)` 钩子）。
 * 上游用它挂「临时访问 PIN」（issue #69/#90）；本 fork 暂未提供替代令牌来源，
 * 但三条通道（cookie / `?token=` / POST 登录）统一走这里，后续接入只改一处。
 */
function acceptedTokensFor(auth, host, token) {
  if (!token) return [];
  const alt = typeof auth?.getAltTokens === 'function' ? (auth.getAltTokens(host) ?? []) : [];
  return [token, ...alt.filter(Boolean)];
}

/** 请求是否期望 HTML（浏览器导航 → 返回登录页；API/WS → 401）。 */
/** 桌面端浏览器访问门禁提示页（issue #81 / 上游 ff003b6）：DSH Desktop 未开启
 *  「浏览器访问」时，上游 desktop-browser-access 门禁对普通浏览器（含经本代理转发的
 *  手机）返回 403 text/plain `forbidden`，且本代理无法携带 Electron renderer secret
 *  绕过（我们也不该去猜 secret）。对符合该特征的浏览器导航请求返回此可操作提示页；
 *  API/WS 与其余 403 原样透传。 */
...
function isHtmlRequest(req) {
  ...
  // 那些同样是浏览器导航，漏判会让它们拿到 401/303/429 而不是该给的页面
  // （issue #90 的锁定态页面、#91 的过渡页与提示页都依赖这里）。
  let pathname = url;
 ...
 * 栅栏（settings.describe / credentials.* / llm.discoverModels 等 PRIVILEGED_METHODS）
 * 会拒绝 sec-fetch-site === 'cross-site'，并校验 Origin 与 Host 匹配；经代理的远程访问
 * 若不改写这两个头，设置/凭据平面会在宿主侧被 403（issue #58 的另一半）。
...
export function createPocketProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, auth = null, rateLimit = null, heartbeat = {}, lanAccessEnabled = () => true, launchToken = () => '', handshakeLimit } = {}) {
  ...
  const server = createServer((req, res) => {
    ...
    if (auth) {
      ...
      if (protectedHost && token) {
        ...
        // 本 fork 目前只有主 PIN（上游的 getAltTokens「临时 PIN」钩子未接入来源），
        // 统一成数组形态与 authCheck 的入参一致，后续加替代令牌只改 acceptedTokensFor。
        const acceptedTokens = acceptedTokensFor(auth, host, token);
        ...
        if (req.method === 'POST' && req.url?.startsWith('/pocket-login')) {
          ...
          req.on('end', () => {
            ...
            // （230039f）。否则攻击者可预先挂起大量 POST，再同时结束请求体，
            // 让它们全部越过入口处那次 status() 检查，批量穿透失败阈值。
    ...
    // 让请求落到提示页，而不是继续转圈。
    //
    // 注：上游此处还会先 stripDesktopMarkers() 清掉历史遗留的 dsh-desktop-* 参数
    // （issue #75）。本 fork 的方向相反——这些参数是 desktopEnvPatchScript 特意补的
    // （见 lib/index.js，缺了会让 dsh-plugin-desktop client 崩溃，issue #3/#4），
    // 因此**不清**，原样透传。
    ...
    const proxyReq = httpRequest(
      ...
      (proxyRes) => {
        ...
        // issue #81（上游 ff003b6）：DSH Desktop 未开启「浏览器访问」时，上游
        // desktop-browser-access 门禁对普通浏览器（含经本代理转发的手机）返回
        // 403 text/plain `forbidden`，本代理无法携带 Electron renderer secret 绕过。
        // 对符合该特征的**浏览器导航**请求改写为可操作提示页；API/WS 与其余 403
        // 原样透传（不猜 secret、不把任意 403 都当桌面门禁）。
        // 上游用 drip 边收边判；这里直接缓冲判断——门禁 body 只有 9 字节，
        // 超过 64KB 立即放弃改写、原样透传，不会把大响应体拖进内存。
        ...
        if (injectHtml && contentType.includes('text/html') && !isCompressed(proxyRes.headers)) {
          ...
          proxyRes.on('end', () => {
            ...
            // 注入后的 HTML 携带本代理的动态补丁（含注入标记判重），必须禁用缓存——
            // 否则手机/中间层（nginx 等）会继续拿没有补丁的旧页面（issue #58）。
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    if (auth) {
      ...
      const wsAccepted = acceptedTokensFor(auth, host, token);
      const wsAuth = authCheck(req, wsAccepted, auth.sessionKey ?? null);
      if (token && !wsAuth.ok) {
        if (wsGuess) {
          limiter?.record(wsIp);
