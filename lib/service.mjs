...
export function createPocketService({
  ...
  return {
    ...
    async restoreFrpIfNeeded() {
      ...
      if (!has) return;
      if (!frpConfigOrNull()) {
        console.warn('dsh-pocket: frp auto-restore skipped — config incomplete | frp 自动恢复跳过：配置不完整');
        return;
      }
      try {
        await this.startFrpTunnel();
        console.log('dsh-pocket: frp tunnel auto-restored | 已自动恢复 NAS 反向隧道');
      } catch (err) {
        console.warn('dsh-pocket: frp auto-restore failed | 自动恢复 frp 隧道失败: %s', err?.message ?? err);
      }
    },
