...
| Want a different port (issue #70) | Plugin mode: write `"proxyPort": 3082` into `$DSH_HOME/dsh-pocket/settings.json` and restart `dsh web`. CLI mode: `dsh-pocket --port 3082`. If the port is taken the proxy scans forward for a free one (+1..+9) and logs the actual port |
...
| Version stuck below 1.x | Use the git `add` command from "Upgrading" above — the fixed `releases/latest` URL / tgz link gets cached as the old package by pnpm |
