...
import { lanAuthEnabled, setLanAuthEnabled, lanIpOverride, setLanIpOverride, pinCustom, setPinCustom, frpConfig, setFrpConfig, frpToken, setFrpToken, resetSettings, lanEnabled, setLanEnabled, proxyPort } from './settings.mjs';
...
/**
 * 恢复出厂设置（设置页底部按钮，672b31b）：清空本机设置 + 重设随机访问密码。
 *
 * 删除 settings.json 与 frp-token → 所有开关回到默认（局域网访问开、访问密码开、
 * 局域网地址自动、公网模式随机、固定域名/Tunnel Token/NAS 配置清空）；公网与局域网
 * 密码都换新随机 8 位值（旧密码立即作废，手机需重新输入）。只动
 * $DSH_HOME/dsh-pocket/ 下的文件，DSH 自身的会话、模型、插件配置不受影响。
 *
 * 注意：调用方应先停掉正在运行的公网隧道与 NAS 反向隧道（RPC 层已处理），
 * 避免重置后仍按旧配置连接。
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  const service = internals.service ?? createPocketService({
    ...
    // 局域网访问总开关（PR #61）：实时读，设置页切一下代理立即拒绝/放行局域网 Host
  ...
  try {
    disposeRpc = installPocketRpc(ctx, {
      ...
      // 恢复出厂设置（672b31b）：清空设置文件 + 重设随机密码（RPC 层已先停掉隧道）
