  ...
  return ctx.connection.rpc.handle(POCKET_RPC_CHANNEL, async (endpoint, payload = {}, signal) => {
    ...
    try {
      ...
      if (endpoint === POCKET_ENDPOINTS.frpConfigGet) {
        ...
        // token 绝不回传，只回传 hasToken
        return ok({ config: { serverAddr: cfg.serverAddr, serverPort: cfg.serverPort, remotePort: cfg.remotePort, tls: cfg.tls === true }, hasToken: Boolean(cfg.token) });
      }
